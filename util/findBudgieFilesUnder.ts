...
/**
 * Retrieves child .bg files within directories.
 *
 ...
 * @returns Child .bg files within the directories.
 */
export const findBudgieFilesUnder = (rootPath: string, directoryNames: string[], prefix: string = "") => {
    const tests = new Map<string, string[]>();

    for (const childName of directoryNames) {
        const directoryPath = path.resolve(rootPath, childName, prefix);
        const testFiles = glob
            .sync(`${directoryPath}/**/*.bg`)
            .map((testFileName) => testFileName.substring(directoryPath.length + 1, testFileName.indexOf(".bg")));
